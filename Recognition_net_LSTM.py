...
from cor_src.cor_LSTM_network import cor_LSTM_resnet34
...
parser.add_argument('--save_folder', default= './cor_RESULTS_comparison/Modified/',type=str, help='Dir to save results')
...
if __name__ == '__main__':
    ...
    NUM_CLASSES = 100
    dataset_lowest_age = 0
    frame_num = 20

    weights_path = r'./cor_weights/LSTM/LSTM_Adam__lr_0.0005__bs_20epoch_count200.pt'
    CORAL_net = cor_LSTM_resnet34(NUM_CLASSES + 1, frame_num = frame_num , grayscale= False)
    ...
    cor_images = torch.Tensor().to(device)
        ...
        for filename in filenames:
            if filename.endswith(".png") or filename.endswith(".jpg") or filename.endswith(".tiff"):
                ...
                with open(save_name, "w") as fd:
                    ...
                    cor_images = torch.cat((cor_images,image),dim=0)

                    if cor_images.shape[0] >= 20:

                        _t['COR_forward_pass'].tic()
                        logits, probas = CORAL_net(cor_images)  # forward pass
                        _t['COR_forward_pass'].toc()

                        cor_images = cor_images[1:,:,:,:]

                        predict_levels = probas > 0.5
                        predicted_label = torch.sum(predict_levels, dim=1)
                        age_text = str(predicted_label.item() + dataset_lowest_age)
                    else:
                        age_text = 'Need to process 20 images first'
                ...
                # save image
                name = save_directory + '\\'+ filename[:-4] + ".jpg"
